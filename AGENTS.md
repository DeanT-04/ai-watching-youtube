...
- **Windows path gotcha for PowerShell/WinRT scripts:** when invoking `scripts/ocr.ps1` (Windows OCR) from a git-bash/MSYS shell, pass a real Windows path — `$(cygpath -w "$(pwd)/<file>")`. MSYS-style `/c/...` paths make `GetFileFromPathAsync` throw, so OCR fails with a misleading "One or more errors occurred"; the OCR engine and language pack are usually fine — the path is the problem.
